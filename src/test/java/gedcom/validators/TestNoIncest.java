...
public class TestNoIncest {
	...
	public void testSiblingIncest() {
		...
		ged = new GEDFile(ged.getIndividuals().toArray(new Individual[ged.getIndividuals().size()]), new Family[] {ged.getFamilies().get(0), fam2});
	...
	public void testFirstCousinIncest() {
		...
		Family subFamily1 = new Family("sub1"),
				subFamily2 = new Family("sub2");
		...
		for(Individual i : indivs) newIndivs.add(i);
		newIndivs.add(j1);
		newIndivs.add(j2);
		
		ged = new GEDFile(newIndivs.toArray(new Individual[newIndivs.size()]), new Family[] {ged.getFamilies().get(0), subFamily1, subFamily2, testFam});
		assertFalse(noIncest.isValid(ged));
	}
